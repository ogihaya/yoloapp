...
- `yolov9/` フォルダー: 動作確認済みのYOLOv9実装が含まれています
  - **推論の実行手順**(以下は別のプロジェクトでyolov9内で行う事例です。アプリに組み込む際は以下の通りにする必要はないです):
    1. 推論したい画像を `yolov9/dataset/images/inference` フォルダーに配置します
    2. `yolov9/yolo/config` フォルダー内の `general.yaml` と `task/inference.yaml` の設定を行います(GPUがない場合はデバイスの設定も変える必要あり)
