...
def create_optimizer(model: YOLO, optim_cfg: OptimizerConfig) -> Optimizer:
    ...
    This function supports SGD (with a custom linear scheduler) and AdamW.
    The choice is determined by `optim_cfg.type`.

    Returns:
        An instance of the optimizer configured according to the provided settings.
    """
    optimizer_name = optim_cfg.type
    optimizer_class: Type[Optimizer] = getattr(torch.optim, optimizer_name)

    # パラメータを「weight decayを適用するグループ」と「しないグループ」に分ける
    params_decay = []
    params_no_decay = []
    for name, p in model.named_parameters():
        if not p.requires_grad:
            continue
        # .bias やバッチ正規化層のパラメータは weight_decay の対象外とする
        if ".bias" in name or ".bn" in name:
            params_no_decay.append(p)
        else:
            params_decay.append(p)

    # --- optim_cfg.type に応じて処理を分岐 ---

    if optimizer_name == "AdamW":
        # AdamW を使用する場合
        # momentumは使わないため、パラメータグループはシンプルになる
        model_parameters = [
            {'params': params_decay},
            {'params': params_no_decay, 'weight_decay': 0.0} # このグループはweight_decayを無効化
        ]
        # lr, betas, weight_decay などの引数は optim_cfg.args から渡す
        optimizer = optimizer_class(model_parameters, **optim_cfg.args)

    elif optimizer_name == "SGD":
        # 従来のSGD（カスタムスケジューラ付き）を使用する場合
        # 元のコードと同様に3つのグループに分ける
        bias_params = [p for name, p in model.named_parameters() if p.requires_grad and "bias" in name]
        norm_params = [p for name, p in model.named_parameters() if p.requires_grad and "weight" in name and "bn" in name]
        conv_params = [p for name, p in model.named_parameters() if p.requires_grad and "weight" in name and "bn" not in name]

        # weight_decayはconv_paramsにのみ適用し、他は0に設定
        model_parameters = [
            {"params": conv_params},
            {"params": bias_params, "weight_decay": 0.0},
            {"params": norm_params, "weight_decay": 0.0},
        ]
        # lr, momentum, weight_decay などの引数は optim_cfg.args から渡す
        optimizer = optimizer_class(model_parameters, **optim_cfg.args)

        # --- カスタムスケジューラ用メソッドを定義 ---
        def next_epoch(self, batch_num, epoch_idx):
            self.min_lr = [param["lr"] for param in self.param_groups]
            # TODO: max_lrはconfigから取得するか、スケジューラで計算するべき
            target_lr = self.defaults['lr']
            self.max_lr = [target_lr] * len(self.param_groups)

            # ウォームアップ（最初の3エポック）中のモメンタムを調整
            warmup_epochs = 3
            start_momentum = 0.937
            normal_momentum = 0.8
            self.min_mom = lerp(normal_momentum, start_momentum, min(epoch_idx, warmup_epochs), warmup_epochs)
            self.max_mom = lerp(normal_momentum, start_momentum, min(epoch_idx + 1, warmup_epochs), warmup_epochs)
        ...
        def next_batch(self):
            ...
            for i, param_group in enumerate(self.param_groups):
                # バッチごとに学習率とモメンタムを線形補間
                min_lr, max_lr = self.min_lr[i], self.max_lr[i]
                ...
                lr_dict[f"LR/{i}"] = param_group["lr"]
                lr_dict[f"momentum/{i}"] = param_group["momentum"]
            return lr_dict

        # 生成したoptimizerインスタンスにのみメソッドを追加（より安全な方法）
        optimizer.next_epoch = types.MethodType(next_epoch, optimizer)
        optimizer.next_batch = types.MethodType(next_batch, optimizer)
        # 初期状態を設定
        optimizer.min_lr = [0.0] * len(model_parameters)

    else:
        # サポートしていないオプティマイザが指定された場合はエラーを出す
        raise ValueError(f"Optimizer '{optimizer_name}' is not supported. Please choose 'SGD' or 'AdamW'.")
...
def create_scheduler(optimizer: Optimizer, schedule_cfg: SchedulerConfig) -> _LRScheduler:
    ...
    if hasattr(schedule_cfg, "warmup"):
        ...
        # 1. オプティマイザから実際のパラメータグループ数を取得
        num_param_groups = len(optimizer.param_groups)

        # 2. グループ数に応じて動的にラムダ関数のリストを生成
        #    (最初のグループにlambda2を、残りにlambda1を適用)
        lr_lambdas = [lambda2] + [lambda1] * (num_param_groups - 1)
        
        # 3. 動的に生成したリストを使ってスケジューラを作成
