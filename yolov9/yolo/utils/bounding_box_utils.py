...
class Vec2Box:
    def __init__(self, model: YOLO, anchor_cfg: AnchorConfig, image_size, device, is_inference: bool = False):
        self.device = device
        ...
        if hasattr(anchor_cfg, "strides"):
            ...
            self.strides = self.create_auto_anchor(model, image_size, is_inference)

        anchor_grid, scaler = generate_anchors(image_size, self.strides)
        self.image_size = image_size
        self.anchor_grid, self.scaler = anchor_grid.to(device), scaler.to(device)

    def create_auto_anchor(self, model: YOLO, image_size, is_inference: bool):
        W, H = image_size
