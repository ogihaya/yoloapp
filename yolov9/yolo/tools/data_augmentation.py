...
class AugmentationComposer:
    ...
    def __init__(self, transforms, image_size: List[int] = [640, 640], base_size: int = 640):
...
@dataclass
class CustomAugmentationConfig:
    prob: float = 0.5
    factor_range: List[float] = field(default_factory=lambda: [0.7, 1.3])

class RandomBrightness:
    """Randomly adjusts the brightness of an image."""

    def __init__(self, config: CustomAugmentationConfig):
        self.prob = float(config.prob)
        self.factor_range = config.factor_range

    def __call__(self, image, boxes):
        if torch.rand(1).item() < self.prob:
            # factor_rangeからランダムに値を選択
            factor = torch.empty(1).uniform_(self.factor_range[0], self.factor_range[1]).item()
