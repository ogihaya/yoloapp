...
def draw_bboxes(
    ...
    *,
    ...
    font_paths = [
        ...
        "/System/Library/Fonts/Hiragino Sans.ttc",  # macOS
    ...
    for font_path in font_paths:
        try:
            font = ImageFont.truetype(font_path, int(label_size))
            break
    ...
    for bbox in bboxes:
        ...
        label_text = f"{class_text}" + (f" {conf[0]:.1%}" if conf else "")
        ...
        try:
            ...
            # フォールバック：固定サイズを使用
            text_width = len(label_text) * int(label_size) * 0.6
            text_height = int(label_size) * 1.2

        # テキスト背景の位置を調整（画像境界内に収める）
        text_x = max(0, min(x_min, img.size[0] - text_width))
        text_y = max(0, min(y_min, img.size[1] - text_height))
