...
- **保存機能**:
  ...
  - **一括保存機能**:
    - **ボタン**: 「すべて保存」ボタンまたは「一括保存」ボタンを画面に配置
    - **動作**: ボタンをクリックすると、すべての推論結果画像が一括で保存される
    - **保存形式**: 
      - 推論結果画像をZIPファイルとして一括ダウンロード（`inference_results.zip`など）
    ...
    - **保存先**: ブラウザのデフォルトダウンロードフォルダ（通常はユーザーのダウンロードフォルダ）
    - **保存対象**: 推論実行済みのすべての画像の推論結果（推論が実行されていない画像は除外）
